...
public class backUpandGrab extends SequentialCommandGroup {
  /** Creates a new backUpandGrab. */
  public backUpandGrab() {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(new raiseArmAndDrop(), new PIDdrive(driveTrain.geDriveTrain(), new PIDValues(3.75, .1, 0), Units.inchesToMeters(-48)).withTimeout(2), new driveUntilAngle(), new PIDdrive(driveTrain.getDriveTrainInstance(), new PIDValues(2.55, .1, 0), Units.inchesToMeters(-87)).andThen(new PIDTurn(driveTrain.geDriveTrain(), new PIDValues(0.001, 0.0005, 0), 167)).andThen(new pivotTrapezoid(20)).andThen(new linearTrapezoid(8) ));
  }
}
