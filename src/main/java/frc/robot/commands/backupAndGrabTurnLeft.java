...
public class backupAndGrabTurnLeft extends SequentialCommandGroup {
  ...
  public backupAndGrabTurnLeft() {
    ...
    addCommands(new raiseArmAndDrop().andThen(new PIDdrive(driveTrain.geDriveTrain(), new PIDValues(2.55, .1, 0), Units.inchesToMeters(-191)).withTimeout(4)).andThen(new PIDTurn(driveTrain.geDriveTrain(), new PIDValues(0.001, 0.0005, 0), 167).withTimeout(2.5)).andThen(new pivotTrapezoid(20)).andThen(new linearTrapezoid(8)));
  }
}
